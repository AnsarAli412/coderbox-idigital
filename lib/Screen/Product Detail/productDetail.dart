...
import 'package:idigital_costumer/Screen/Product%20Detail/Widget/productExtraDetail.dart';
import 'package:idigital_costumer/widgets/star_rating.dart';
...
import 'package:idigital_costumer/Model/User.dart';
...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    print('extra desc***${widget.model!.extraDesc}');
                              ...
                              onTap: () async {
                                if (validateAndSave()) {
                                  if (IS_SHIPROCKET_ON == '1') {
                                    validatePinFromShipRocket(curPin, true);
                                  } else {
 ...
 /* _deliverPincode() {
  ...
  Future<void> validatePinFromShipRocket(String pin, bool wantsToPop) async {
    try {
      isNetworkAvail = await isNetworkAvailable();
      if (isNetworkAvail) {
        try {
  ...
  Future<void> getDeliverable() async {
    ...
    if (pin != '') {
      if (IS_SHIPROCKET_ON == '1') {
        validatePinFromShipRocket(pin, false);
      } else {
