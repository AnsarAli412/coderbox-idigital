...
class StateCart extends State<Cart> with TickerProviderStateMixin {
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      ...
                                      setState(() {});
                                                    ...
                                                    } else if (tempCartListForTestCondtion[0]
                                                                .productList![0]
                                                                .productType !=
                                                            'digital_product' &&
                                                        !context.read<CartProvider>().deliverable) {
      ...
      if (context.read<CartProvider>().addressList.isNotEmpty) {
        try {
          ...
          CartRepository.fetchUserCart(parameter: parameter).then((getData) {
            ...
            if (error) {
              setSnackbar(msg.toString(), _checkscaffoldKey);
              context.read<CartProvider>().checkoutState!(() {
                // _placeOrder = false;
                context.read<CartProvider>().deliverable = false;
              });
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    ...
    if (mounted) {
      setSnackbar(errorMsg /*response.message!.toString()*/, _checkscaffoldKey);
    }
    deleteOrder(orderId);
    if (mounted) {
      context.read<CartProvider>().checkoutState!(
